...
//        throw new RuntimeException("Implement creation of function prologue");
    	prologue.add("subu $sp, $sp, 8");  //callee first allocates some space for bookkeeping values.
    	prologue.add("sw $fp, 0($sp)");  //callee saves the caller's frame pointer and return address.
    	prologue.add("sw $ra, 4($sp)");
    	prologue.add("addi $fp, $sp, 8");  // updates the frame pointer to reference its own activation record
